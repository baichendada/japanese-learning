...
test('learner can open practice, start with Space, type, and inspect levels', async ({ page }) => {
  ...
  await expect(page.getByText('挑战中')).toBeVisible();
  ...
  const drawer = page.getByRole('dialog', { name: '关卡列表' });
  await expect(drawer).toBeVisible();
  await drawer.getByLabel('选择课程').selectOption('katakana-basic');
  await drawer.getByRole('button', { name: '选择关卡 ア行' }).click();

  await expect(page.getByText('1/20 ア行')).toBeVisible();
});
...
test('Space activates focused level controls without starting practice globally', async ({ page }) => {
  ...
  await expect(page.getByText('挑战中')).not.toBeVisible();
});
...
test('Tab on a focused control does not restart the running practice session', async ({ page }) => {
  ...
  const toolbarButton = page.getByRole('button', { name: '易混淆' });
